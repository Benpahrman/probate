...
const getInitialCorrectionType = (type: string): string => {
...
const buildAuditDetails = (params: CorrectionState): string => {
  const { correctionType, apnOverride, lettersStatus, contactLine, notes } = params;
...
const ExceptionDetailsSummary: React.FC<{ exception: ExceptionItem }> = (props) => {
...
interface CorrectionFormFieldsProps {
  formState: CorrectionState;
  onUpdate: (key: keyof CorrectionState, val: string) => void;
}

const CorrectionFormFields: React.FC<CorrectionFormFieldsProps> = (props) => {
  const { formState, onUpdate } = props;
  const { correctionType, apnOverride, lettersStatus, contactLine, notes } = formState;
    ...
    <div className="space-y-3">
      <div>
        ...
        <select
          ...
          onChange={(e) => onUpdate('correctionType', e.target.value)}
      ...
      {correctionType === 'APN_OVERRIDE' && (
        <div>
          ...
          <input
            ...
            value={apnOverride}
            onChange={(e) => onUpdate('apnOverride', e.target.value)}
            className="w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-xs text-white font-mono focus:outline-none focus:border-indigo-500"
          />
        </div>
      )}
      ...
      {correctionType === 'LETTERS_CLASSIFICATION' && (
        <div>
          ...
          <select
            ...
            value={lettersStatus}
            onChange={(e) => onUpdate('lettersStatus', e.target.value)}
            className="w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-xs text-white font-mono focus:outline-none focus:border-indigo-500"
          >
      ...
      {correctionType === 'CONTACT_REPLACEMENT' && (
        <div>
          ...
          <input
            ...
            value={contactLine}
            onChange={(e) => onUpdate('contactLine', e.target.value)}
            className="w-full bg-slate-950 border border-slate-700 rounded-lg px-3 py-2 text-xs text-white font-mono focus:outline-none focus:border-indigo-500"
          />
        </div>
      )}
...
const ResolutionActionFooter: React.FC<{ submitting: boolean; onCancel: () => void }> = (props) => (
  <div className="flex justify-end space-x-3 pt-3 border-t border-slate-800">
    ...
    >
      {props.submitting ? 'Committing Triage...' : 'Commit Resolution & Re-Audit 6 Gates'}
    </button>
  </div>
);

export const ExceptionResolve: React.FC<ExceptionResolveProps> = (props) => {
  const { exception, onCommit, onCancel, onReAudit } = props;

  const [formState, setFormState] = useState<CorrectionState>(() => ({
    correctionType: getInitialCorrectionType(exception.type),
    apnOverride: '',
    lettersStatus: 'CONFIRMED_NONINTERVENTION',
    contactLine: '',
    notes: '',
  }));
  ...
  const handleFieldUpdate = (key: keyof CorrectionState, val: string) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const auditDetails = buildAuditDetails(formState);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <CorrectionFormFields formState={formState} onUpdate={handleFieldUpdate} />
      <ResolutionActionFooter submitting={submitting} onCancel={onCancel} />
    </form>
  );
};
